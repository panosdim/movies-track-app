...
def connect_to_db():
    ...
    Returns:
        connection (psycopg2.connection): The PostgreSQL database connection object if successful, None otherwise.
    """
    try:
...
def fetch_data_from_db(query: str, params: Tuple = ()) -> List[Tuple]:
    ...
    if connection is None:
        _logger.warning("PostgreSQL Connection not available.")
        return []
...
def get_movie_ratings(user_id: str) -> Tuple[List[int], List[float]]:
    ...
    data = fetch_data_from_db("SELECT movie_id, rating FROM movie WHERE user_id = %s", (user_id,))
...
def get_movie_ids(user_id: str) -> List[int]:
    ...
    """
    data = fetch_data_from_db("SELECT movie_id FROM movie WHERE user_id = %s", (user_id,))
